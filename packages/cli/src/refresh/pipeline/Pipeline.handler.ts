...
import { pipelineScan, ICMCIApiResponse } from "@zowe/cics-for-zowe-sdk";
import { CicsBaseHandler } from "../../CicsBaseHandler";

import i18nTypings from "../../-strings-/en";

// Does not use the import in anticipation of some internationalization work to be done later.
const strings = (require("../../-strings-/en").default as typeof i18nTypings).REFRESH.RESOURCES.PIPELINE;

/**
 * Command handler for refreshing CICS pipeline via CMCI
 * @export
...
export default class PipelineHandler extends CicsBaseHandler {
  public async processWithSession(params: IHandlerParameters, session: AbstractSession, profile: IProfile): Promise<ICMCIApiResponse> {
    ...
    const status: ITaskWithStatus = {
      statusMessage: "Refreshing pipeline to CICS",
      percentComplete: 0,
      stageName: TaskStage.IN_PROGRESS
    };
    params.response.progress.startBar({task: status});

    const response = await pipelineScan(session, {
